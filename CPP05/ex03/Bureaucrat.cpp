 ...
 * @brief Implementation of the Bureaucrat class. The class provides functionality to manage the 
 * bureaucrat's grade, including incrementing, decrementing, and checking grade validity.
 * @date 2024-07-29
 *
 * @details The Bureaucrat class represents an employee with a name and a grade. The grade can be
 * incremented or decremented, but must remain within predefined limits. Exceptions are thrown if 
 * the grade is set too high or too low.
 */
 ...
 */
void	Bureaucrat::signForm( AForm &oneForm ) {
	try {
...
/**
 * @brief Executes a form, if the Bureaucrat meets the required grade.
 * 
...
void	Bureaucrat::executeForm( AForm &form ) {
	try {
		...
		std::cerr << YELLOW << _name << " couldn't execute " << form.getName() << " because " << e.what() << RESET << std::endl;
	}
}